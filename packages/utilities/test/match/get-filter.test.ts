...
describe('Utilities', () => {
    ...
    describe('Match get filter', () => {
        ...
        const include = (matcher: Matcher) => getFilter({ include: matcher });
        const exclude = (matcher: Matcher) => getFilter({ exclude: matcher });
        ...
        test('Should exclude items in a matching filter', () => {
            ...
            expect(stringFilter('Item 3')).toBe(false);
            expect(regexFilter('Value 4')).toBe(false);
            expect(regexFilter('Value 5')).toBe(true);
            expect(allFilter('something')).toBe(false);
        });
